...
// Step 4: Fetch User's Top 5 Tracks
app.get("/top-tracks", async (req, res) => {
  ...
  try {
    const [response1, response2] = await Promise.all([
      ...
      global.fetch("https://api.spotify.com/v1/me/top/tracks?offset=50&limit=50", {
        headers: { Authorization: `Bearer ${accessToken}` },
      }),
    ]);
    ...
    const cleanedItems = mergedItems.map((track) => {
      const { available_markets, album, href, is_local, is_playable, preview_url, track_number, type, uri, disc_number, external_ids, external_urls, ...trackWithoutMarkets } = track;
